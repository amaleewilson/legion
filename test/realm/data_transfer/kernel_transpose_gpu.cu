...
__device__ void bp_soa_to_aos(float *h_0, float *h_1, float *h_2, float *h_3, float *d_dst, size_type elem_size, 
  ...
  for (size_type t_id = real_tid; t_id < loop_term; t_id += inc){
    ...
    for (size_type i = 0; i < lt; ++i){
      d_dst[dst_base + 0 + i*fid_count] = h_0[src_base + i];
      d_dst[dst_base + 1 + i*fid_count] = h_1[src_base + i];
      d_dst[dst_base + 2 + i*fid_count] = h_2[src_base + i];
      d_dst[dst_base + 3 + i*fid_count] = h_3[src_base + i];
    }
  }
}
 ...
 */
template <int block_size, typename size_type>
__device__ void bp_aos_to_soa(float *h_0, float *h_1, float *h_2, float *h_3, float *d_dst, size_type elem_size, 
        size_type elem_count, int fid_count, int c_sz) {
  ...
  for (size_type t_id = real_tid; t_id < loop_term; t_id += inc){
    ...
    #pragma unroll 
    for (size_type i = 0; i < lt; ++i){
      d_dst[dst_base0 + t_id*lt + i] = h_0[t_id*c_sz + 0 + i*fid_count];
      d_dst[dst_base1 + t_id*lt + i] = h_0[t_id*c_sz + 1 + i*fid_count];
      d_dst[dst_base2 + t_id*lt + i] = h_0[t_id*c_sz + 2 + i*fid_count];
      d_dst[dst_base3 + t_id*lt + i] = h_0[t_id*c_sz + 3 + i*fid_count];
    }
  }
}
...
extern "C" __global__ void bp_aos_to_soa(float *h_0, float *h_1, float *h_2, float *h_3, float *d_dst,
                                                int e_size, int e_count, int fid_count, int c_sz) {
  bp_aos_to_soa<32, int>(h_0, h_1, h_2, h_3, d_dst, e_size, e_count, fid_count, c_sz);
}
