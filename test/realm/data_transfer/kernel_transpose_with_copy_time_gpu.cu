...
__device__ void bp_aos_to_soa(float *h_0, float *h_1, float *h_2, float *h_3, float *d_dst, size_type elem_size, 
  ...
  for (size_type t_id = real_tid; t_id < loop_term; t_id += inc){
    ...
    #pragma unroll 
    for (size_type i = 0; i < lt; ++i){
      d_dst[dst_base0 + t_id*lt + i] = h_0[src_base + 0 + i*fid_count];
      d_dst[dst_base1 + t_id*lt + i] = h_0[src_base + 1 + i*fid_count];
      d_dst[dst_base2 + t_id*lt + i] = h_0[src_base + 2 + i*fid_count];
      d_dst[dst_base3 + t_id*lt + i] = h_0[src_base + 3 + i*fid_count];
    }
  }
}
